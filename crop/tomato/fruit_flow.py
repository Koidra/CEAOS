...
def fruit_set_of_first_development_stage(_24_canopy_t, carbohydrate_flow_BufFruits):
    """
    Equation 9.29, B.4
    ...
    """
    return smoothed_conditional_function(carbohydrate_flow_BufFruits, -58.9, 0.05) * maximum_fruit_set_dependency_on_temperature(_24_canopy_t)


def maximum_fruit_set_dependency_on_temperature(_24_canopy_t):
    """
    Equation 9.30
    max_number_flow_BufFruit_1 = PLANT_DENSITY * (MAX_FRUIT_SET_REGRESSION_COEF_1 + MAX_FRUIT_SET_REGRESSION_COEF_2*_24_canopy_t)
    Returns: maximum fruit set dependency on temperature [fruits m^-2 s^-1]
    """
    return PLANT_DENSITY * (MAX_FRUIT_SET_REGRESSION_COEF_1 + MAX_FRUIT_SET_REGRESSION_COEF_2 * _24_canopy_t)


def fruit_flow_through_fruit_development_stage(jth: int, number_Fruits, sum_canopy_t, _24_canopy_t):
    """
    Equation 9.31
