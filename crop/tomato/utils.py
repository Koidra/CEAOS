...
def growth_rate_dependency_to_temperature(last_24_canopy_t):
    ...
    """
    return 0.0047 * last_24_canopy_t + 0.06


def fruit_development(last_24_canopy_t):
    """
    Equations 9.32
    fruit_development_rate = FRUIT_DEVELOPMENT_RATE_COEF_1 + FRUIT_DEVELOPMENT_RATE_COEF_2 * last_24_canopy_t
    Returns: fruit development rate [s^-1]
    """
    return FRUIT_DEVELOPMENT_RATE_COEF_1 + FRUIT_DEVELOPMENT_RATE_COEF_2 * last_24_canopy_t
...
def sum_carbohydrate_flow_BufFruit_conversion(number_Fruits, last_24_canopy_t):
    """
    Equations 9.37
    ...
    for i in range(1, FRUIT_DEVELOPMENT_STAGES_NUM):
        total += number_Fruits[i] * fruit_growth(i, last_24_canopy_t)
    return 1/total
